...
sudo sed -i 's@#indicators=@indicators=~spacer;~clock;~spacer;~session;~a11y;~power@g' /etc/lightdm/lightdm-gtk-greeter.conf
#sudo rm /var/cache/lightdm/dmrc/*
sudo mkdir -p /etc/lightdm/lightdm.conf.d
cat <<EOF |sudo tee /etc/lightdm/lightdm.conf.d/seat.conf>/dev/null
[Seat:*]
